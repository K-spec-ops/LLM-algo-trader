...
You can check that the environment variable is set correctly using: echo $OPEN_AI_KEY."""
    ...
    parser.add_argument("-c", "--cont", action= "store_true", default= None, help= "Run iterations on an EC2 instance; default False, autoselects True for '-n'> 25")
...
def run_wrapper(command, tout):
    ...
    while time()< end:
        code= p.poll()
        logger.debug(f"sub-process has a return code of {code}")
        if code== 0:
            break
        sleep(0.5)
    else:
        logger.debug(f"sub-process didn't complete in {tout} seconds. Exiting...")
        p.terminate()
        try:
            p.wait(timeout= 10)
...
parent= make_logger(__name__, logging.INFO, fn if cont else None)
...
if cont:
    ...
    try:
        ...
        parent.error("Your AWS credentials are invalid, expired, or missing!")
        sys.exit(1)
    ...
    info= ec2.run_instances(ImageId= ami, 
                         ...
                         UserData= '''#!/bin/bash 
                            ...
                            python file.py''')["Instances"][0]
    # code_1= run_wrapper(["chmod", "+x", "traderbash.sh"])
    # if code_1!= 0:
    #     parent.error(f"Could not grant execute permissions to the bash script! Error code {code_1}")
    #     sys.exit(1)
    ...
    parent.info(f'''Instance {instanceid} has launched! DETAILS: 
                                                  Architechure: {arch}
                                                  State: {state}
                                                  Platform: {platform}
                                                  Instance type: {typ}
